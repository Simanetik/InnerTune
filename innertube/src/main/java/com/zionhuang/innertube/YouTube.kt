...
object YouTube {
    ...
    suspend fun player(videoId: String, playlistId: String? = null): Result<PlayerResponse> = runCatching {
        ...
        // Use ANDROID_VR as primary client (no auth required, streams not 403'd)
        playerResponse = innerTube.player(ANDROID_VR, videoId, playlistId).body<PlayerResponse>()
        ...
        // Try refreshing visitorData and retrying
        val freshVisitorData = visitorData().getOrNull()
        if (freshVisitorData != null && freshVisitorData != innerTube.visitorData) {
            innerTube.visitorData = freshVisitorData
            playerResponse = innerTube.player(ANDROID_VR, videoId, playlistId).body<PlayerResponse>()
            if (playerResponse.playabilityStatus.status == "OK") {
                return@runCatching playerResponse
            }
